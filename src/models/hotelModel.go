...
const (
	...
	getHotelByTrip = "SELECT * FROM hotels INNER JOIN trips_hotels ON hotels.id=trips_hotels.hotel_id AND trips_hotels.trip_id=$1"
)
...
var AddHotelToTrip = func(hotelID uuid.UUID, tripID uuid.UUID) error {
	_, err := DB.Exec(addHotelToTrip, hotelID, tripID)
	return err
}
