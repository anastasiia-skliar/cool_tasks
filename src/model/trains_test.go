...
var originalGenerator = model.SQLGenerator
...
func TestGetTrains(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		model.SQLGenerator = originalGenerator
		result, err := model.GetTrains(params)
...
func TestSaveTrainToTrip(t *testing.T) {
	...
	if err := model.AddTrainToTrip(trainID, tripID); err != nil {
		t.Errorf("error was not expected while updating stats: %s", err)
	}
...
func TestGetTrainFromTrip(t *testing.T) {
	...
	result, err := model.GetTrainsFromTrip(ID)
